...
class Kijiji():
    ...
    def __init__(self, headless=False):
        ...
        self.get("homepage")
        language_toggle = self._driver.find_element_by_xpath(
            "//a[@data-qa-id='language-toggle']")
    ...
    def login(self, username, password):
        ...
        email_input = self._driver.find_element_by_xpath(
            "//input[@name='emailOrNickname']")
    ...
    def logout(self):
        if self._logged_in:
            ...
            if self.wait_element_present(("xpath", "//button[@aria-label='Navigation menu button']")):
                return 1
            dropdown = self._driver.find_element_by_xpath(
                "//button[@aria-label='Navigation menu button']")
            if dropdown.get_attribute("aria-expanded") == "false":
                dropdown.click()
            dropdown.find_element_by_xpath(
                "//button[text()='Log Out']").click()
            del self._username
            del self._password
        self._logged_in = False
    ...
    def URLs_fetch(self, name):
        ...
        assert (name in ("homepage", "login")
                ), "Fetching '%s' not implemented (yet)" % str(name)
    ...
    def wait_element_present(self, locator, timeout=5):
        assert (type(locator) is tuple and len(locator) ==
                2), "Invalid locator: %s" % str(locator)
        try:
            WebDriverWait(self._driver, timeout).until(
